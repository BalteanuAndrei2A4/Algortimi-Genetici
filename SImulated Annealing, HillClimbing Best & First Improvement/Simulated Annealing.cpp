...
int main()
{
	...
	double a = -5.12;
	double b = 5.12;
	int D = 30;//aici se inlocuieste cu 10 sau 30 in functie de cate dimensiuni are functia
	long double L = b - a;
	L = L * pow(10, 5);
	L = log2(L);
	...
	uniform_real_distribution<> dis(0, L - 1);
	bool ok = 0;
	...
	for (double T = 1000; T > 1; T *= .995)
	{
		for (int i = 0; i < 100; i++)
		{
			...
			while (local == false)
			{
				...
				for (int j = 0; j < L; j++)
				{
					int pozitie = int(dis(g_randomGenerator));
					vc[pozitie] = !vc[pozitie];
